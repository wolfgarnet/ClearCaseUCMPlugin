	...
	public String parse( State state, String args ) {
		...
		try {
			...
			if( !state.getBaseline().isLoaded() ) {
				//bl = (Baseline) RemoteUtil.loadEntity( state.getWorkspace(), null, state.getBaseline(), true );
			} else {
			...
			if( !bl.getStream().isLoaded() ) {
				//st = (Stream) RemoteUtil.loadEntity( state.getWorkspace(), null, state.getBaseline().getStream(), true );
			} else {
				st = bl.getStream();
			}
