...
public class SelfModeTest extends BaseTestClass {
    ...
    @Test
    @ClearCaseUniqueVobName( name = "promote-fail" )
    public void promoteFail() throws IOException, ExecutionException, InterruptedException, ClearCaseException {
        Stream stream = ccenv.context.streams.get( "one_int" );
        Component component = ccenv.context.components.get( "_System" );
        Project.PromotionLevel level = Project.PromotionLevel.INITIAL;
        ...
        new SystemValidator( build ).validateBuild( Result.UNSTABLE ).validateBuiltBaseline( Project.PromotionLevel.INITIAL, baseline ).validate();
    }
}
